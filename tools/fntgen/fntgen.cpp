...
int convert(const char* input_file, const char* output_file, const char* ttf_file) {
  FILE*            input;
  ...
  if (f == NULL) {
    cerr << "FATAL ERROR: cannot open font %s: " << ttf_file << SDL_GetError() << endl;
    return 1;
  }
  ...
  if (input == NULL) {
    cerr << "FATAL ERROR: cannot open input file " << input_file << endl;
    return 1;
  }
  ...
  if (Encode(
          output_file,
          (unsigned char*)&fntheader,
          sizeof(fntfile_header_t),
          (unsigned char*)chars,
