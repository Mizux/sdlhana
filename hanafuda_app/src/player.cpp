...
void CBasePlayer::CalcResult() {
  ...
  if (gpGame->GetGameMode() == GAMEMODE_KOREAN) {
    ...
    if (m_Result.score == 0 && m_iNumHandCard == 0 && GetOpponent()->GetNumHandCard() == 0) {
...
#define DRAW_RESULT(r, cards, s)                                                              \
  ...
  unsigned char r_cards[]          = {2,  3,  6,  7,  10, 11, 14, 15, 18, 19, 22, 23, 26,
                                      27, 30, 31, 32, 34, 35, 38, 39, 43, 45, 46, 47, 255};
...
#define DRAW_RESULT_A(r, s)                                                                   \
  ...
  y += 26;
  gpGeneral->DrawText(IsBot() ? msg("computerwin") : msg("youwin"), 30, y, 255, 255, 0, 24);
}
...
void CBasePlayer::DrawHand() {
  ...
  for (i = 0; i < m_iNumHandCard; i++) {
    gpGeneral->DrawCard(
        IsBot() ? CCard(255) : m_HandCards[i], dstrect.x, dstrect.y, dstrect.w, dstrect.h, false);
    dstrect.x += dstrect.w;
  }
...
void CBasePlayer::DrawCaptured() {
  ...
  if (IsBot()) {
    UTIL_FillRect(
        gpScreen, 10 + 48 * m_iNumHandCard, 10, 640 - (10 + 48 * m_iNumHandCard), 78, 30, 130, 100);
    ...
    UTIL_FillRect(
        gpScreen,
        10 + 48 * m_iNumHandCard,
        400,
        640 - (10 + 48 * m_iNumHandCard),
        78,
        30,
        130,
        100);
