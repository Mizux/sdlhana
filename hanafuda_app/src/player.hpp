...
class CBasePlayer {
 public:
  ...
  inline CCard GetHandCard(int i) const {
    assert(i >= 0 && i < m_iNumHandCard);
    return m_HandCards[i];
  }
  inline int   GetNumCapturedCard() const { return m_iNumCapturedCard; }
  inline CCard GetCapturedCard(int i) const {
    assert(i >= 0 && i < m_iNumCapturedCard);
    return m_CapturedCard[i];
  }
