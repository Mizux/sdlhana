...
CIniFile::CIniFile() : ini(NULL), key_count(0), current_size(0) {
#ifdef WITH_HASH
...
CIniFile::CIniFile(const char* filename) : ini(NULL), key_count(0), current_size(0) {
#ifdef WITH_HASH
  m_Hash = (ini_key_t**)calloc(INI_HASH_KEY_SIZE, sizeof(ini_key_t*));
  if (m_Hash == NULL) {
    printf("Memory allocation error !\n");
...
void CIniFile::Set(const char* key, const char* value, const char* set) {
  ...
  if (pValue != NULL) {
    ...
    if (pKey->value_count > pKey->current_size) {
      ...
      if (pKey->values) {
        pKey->values =
            (ini_value_t*)realloc(pKey->values, sizeof(ini_value_t) * pKey->current_size);
      } else {
    ...
    unsigned short vhash =
        GetHashValue(pKey->values[pKey->value_count - 1].value_name) % INI_HASH_VALUE_SIZE;
