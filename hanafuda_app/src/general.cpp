...
void CGeneral::ScreenFade(int duration, SDL_Surface* s) {
  SDL_Surface* pNewFadeSurface = SDL_CreateRGBSurface(
  ...
  if (s == nullptr) {
    // make black screen
    SDL_FillRect(pNewFadeSurface, nullptr, SDL_MapRGB(pNewFadeSurface->format, 0, 0, 0));
  } else {
...
void CGeneral::ClearScreen(bool fadein, bool fadeout, bool bg) {
  SDL_Surface* r = SDL_CreateRGBSurface(
      gpScreen->flags,
      gpScreen->w,
      gpScreen->h,
      gpScreen->format->BitsPerPixel,
  ...
  UTIL_RectShade(r, 0, 0, gpScreen->w, gpScreen->h, 196, 196, 0, 0, 196, 196, 196, 0, 196);
...
SDL_AudioCVT* CGeneral::LoadSoundFile(const char* filename) {
  ...
  if (s == nullptr) {
    TerminateOnError("Cannot load sound file %s: %s", filename, SDL_GetError());
  }
...
CBox::CBox(int x, int y, int w, int h, int r, int g, int b, int a, bool keep) {
  ...
  if (keep) {
    ...
    m_pSavedArea = SDL_CreateRGBSurface(
        gpScreen->flags,
        w,
        h,
        gpScreen->format->BitsPerPixel,
...
CButton::CButton(int id, int x, int y, int w, int h, int r, int g, int b)
    : CBox(x, y, w, h, r, g, b, 128, false), m_iId(id) {
...
int CButton::GetButtonId(int x, int y) {
  ...
  for (i = 0; i < num_buttons; i++) {
    if (x > bx[i] && x < bx[i] + bw[i] && y > by[i] && y < by[i] + bh[i]) {
      return bid[i];
    }
  }
