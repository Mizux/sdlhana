...
CFont::CFont() : m_iNumChar(0), m_pChars(NULL) {}

CFont::CFont(const char* filename) : m_iNumChar(0), m_pChars(NULL) {
  Load(filename);
}
...
int CFont::Load(const char* filename) {
  ...
  Decode(
      ...
      sizeof(fntchar_t) * header.num_chars);
...
SDL_Surface* CFont::Render(const char* sz, int r, int g, int b, int size, bool shadow) {
  ...
  while (*p) {
    ...
    if (c != NULL) {
      ...
      SDL_Surface* char_surface = SDL_CreateRGBSurface(
          s->flags,
          (shadow ? 66 : 64),
          (shadow ? 66 : 64),
          gpScreen->format->BitsPerPixel,
        ...
        for (i = 0; i < 64; i++) {
          for (j = 0; j < 64; j++) {
            if (c->pixeldata[i][j / 8] & (1 << (j & 7))) {
              UTIL_PutPixel(
                  char_surface, j + 2, i + 2, SDL_MapRGBA(char_surface->format, 1, 1, 1, 255));
            }
          }
        }
      }
