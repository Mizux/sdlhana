...
CGame::CGame() {
  ...
  m_pPlayers[0] = m_pPlayers[1] = nullptr;
...
void CGame::Settings() {
  ...
  const char* stras[5] = {
      msg("veryslow"), msg("slow"), msg("middle"), msg("fast"), msg("veryfast")};
...
void CGame::InitGame() {
  ...
  for (int i = 0; i < 48; i++) {
    CCard c(i);
    gpGeneral->DrawCard(c, RandomLong(0, 640 - 48), RandomLong(0, 480 - 78), 48, 78, true);
    UTIL_Delay(5);
  }
}
...
void CGame::PlayRound() {
  ...
  // playing the game
  CBasePlayer *current = CBasePlayer::GetDealer(), *winner = nullptr;
  ...
  while (1) {
    if (current->GetNumHandCard() <= 0) {
      ...
      if (GetGameMode() != GAMEMODE_KOREAN) {
        current->CalcResult();
        if (current->m_Result.score > 0 && current->GetOpponent()->m_Result.score <= 0) {
          winner = current;
        }
      } else {
        winner = nullptr;
    ...
    CardDiscarded(current->GetHandCard(s), current, 10 + 48 * s, (current->IsBot() ? 10 : 400));
...
void CGame::CardDiscarded(const CCard& c, CBasePlayer* current, int sx, int sy) {
  ...
  SDL_Surface *save1 = nullptr, *save2 = nullptr;
  ...
  if (count <= 0) {
    ...
    if (GetGameMode() == GAMEMODE_KOREAN && c == drawn && current->GetNumHandCard() > 1) {
  ...
  SDL_BlitSurface(gpScreen, &dstrect, save2, nullptr);
  ...
  if (count <= 0 || leavethree) {
    ...
    if (save1 != nullptr && save2 != nullptr && (x1 < x2 || y1 < y2)) {
...
int CGame::SelectCardOnDesk(int month) {
  ...
  SDL_BlitSurface(save, nullptr, gpScreen, &dstrect);
  gpGeneral->UpdateScreen(dstrect.x, dstrect.y, dstrect.w, dstrect.h);
...
void CGame::AnimDeal() {
  ...
  gpGeneral->PlaySound(SOUND_HINT);
  gpGeneral->DrawText(
      (CBasePlayer::GetDealer() == m_pPlayers[1]) ? msg("comdealer") : msg("youdealer"),
      30,
      300,
      255,
      255,
...
SDL_Surface* CGame::AnimCardMove(
    int          sx,
    int          sy,
    int          dx,
    int          dy,
...
void CGame::DrawDeskCard() {
  ...
  for (i = 0; i < 20; i++) {
    ...
    if (i < m_iNumDeskCard && m_DeskCards[i].IsValid()) {
      gpGeneral->DrawCard(m_DeskCards[i], dstrect.x, dstrect.y, 48, 78, false);
    } else {
